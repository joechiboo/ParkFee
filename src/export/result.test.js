...
// 公告揭露：工作人員配的是住戶配畢後的剩餘位、住戶申請時即收回，性質與住戶承租不同 → 公開版隱藏。
describe('公開版隱藏工作人員（2026-08-26）', () => {
  ...
  // 2026-09-03 例會否決免費案 → 工作人員付費比照住戶；原本斷言 0 元，改為與住戶同額 1,200。
  it('內部版：工作人員完整保留，且應繳金額比照住戶（1,200）', () => {
    const rows = resultRows(result)
    expect(rows).toHaveLength(3)
    expect(rows.find((r) => r.車號 === 'S-001').應繳金額).toBe(1200)
  })
})
